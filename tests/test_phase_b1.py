...
Migrated to tests/ with pytest skipif guard for CI safety.
...
def test_phase_b1():
    ...
    # ── quota_adjustment: billing gate skipped ────────────────────────────────
    ...
    rs = requests.post(f"{BASE}/auth/login", json={"username": os.environ.get("TEST_SALES_USER", "yehuda"), "password": os.environ.get("TEST_SALES_PASS", "")})
    if rs.ok:
    ...
    assert not errors, f"{len(errors)} check(s) failed: {errors}"
