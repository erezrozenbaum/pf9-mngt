#!/usr/bin/env python3
"""Live cluster verification for v1.93.21 -- run on Windows with kubectl configured."""
import json
import subprocess
import sys
...
for p in pods:
    for c in p["spec"]["containers"]:
        img = c["image"]
        if "ghcr.io" in img and ":v1.93.21" not in img:
            wrong_ver.append(f"{p['metadata']['name']}: {img}")
if not wrong_ver:
    ok("all app images tagged v1.93.21")
else:
...
print()
if FAIL_COUNT == 0:
    print("ALL CHECKS PASSED -- v1.93.21 cluster state is healthy")
    sys.exit(0)
else:
