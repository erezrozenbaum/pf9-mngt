...
### Fixed
- Dependency graph edge visibility in dark mode using CSS custom properties
- Enhanced chargeback calculations validation and testing
...
- **Tenant portal — Chargeback (cost estimation)** — New "Chargeback" screen in the tenant portal shows per-VM estimated cost for the authenticated tenant's own projects (other tenants never visible). Period selector (24 h / 7d / 30d / 90d), display-currency override (USD, EUR, GBP, ILS, …), and expandable per-VM rows showing pricing basis detail. Uses flavor-specific prices from `metering_pricing` when available, falling back to the per-vCPU + per-GB-RAM rates in `metering_config`. A prominent disclaimer states this is an estimation. Requires `SELECT` access for `tenant_portal_role` on `metering_resources`, `metering_config`, `metering_pricing` — grant applied automatically on admin API startup and added to `db/init.sql`.
...
- **VM Provisioning (Linux) — QEMU guest agent installed but Nova still returns 409** — Cloud-init correctly installs and starts `qemu-guest-agent` on first boot, but Nova/libvirt only adds the `virtio-serial` channel device (`org.qemu.guest_agent.0`) to the domain XML when the Glance image has `hw_qemu_guest_agent=yes`. Without this property the channel device is absent from the VM hardware; the agent process runs inside the guest but has nothing to communicate through, so Nova `changePassword` returns HTTP 409 "QEMU guest agent is not enabled." Fixed: the provisioning execution loop now patches Linux images with `hw_qemu_guest_agent=yes` via the Glance v2 JSON-Patch API before creating the boot volume, exactly as Windows images are already patched with `hw_disk_bus`/`hw_firmware_type`. Failures are logged as warnings and do not abort provisioning (`api/vm_provisioning_routes.py`).

- **Runbook Reset VM Password — HTTP 409 QEMU guest agent error shows generic note** — When Nova returns `HTTP 409: QEMU guest agent is not enabled`, the result panel showed the generic note "changePassword may not be supported by this hypervisor/image" with no fix guidance. Now detects the 409 + `guest agent` text specifically and shows actionable install instructions for both Ubuntu/Debian and RHEL/CentOS/Rocky. The pre-emptive `Guest Agent Warning` block that previously appeared for every Linux VM (even on success) is now only added to the result when the password reset actually fails with a guest-agent error (`api/runbook_routes.py`).

- **Monitoring Current Usage — cards show `—` for CPU, RAM, and Storage** — When Prometheus exporters are not configured, the DB allocation fallback in `metrics_all_vms()` returned the correct VM list but the `vcpus` field was not included in the records dict passed to `_normalize_vm_entry()`. The UI `MetricItem` component only rendered progress bars when a live percentage was available and fell back to `—` otherwise. Fixed: `vcpus` is now forwarded through `_normalize_vm_entry()` and the allocation fallback path; the UI `MetricItem` accepts an optional `alloc` string that is displayed when no live percentage is available (e.g. `2 vCPU`, `4 GB`, `20 GB`). A banner is shown on the Current Usage tab when data comes from the allocation source, explaining that live CPU/RAM usage requires Prometheus node-exporter (`tenant_portal/metrics_routes.py`, `tenant-ui/src/screens/Monitoring.tsx`, `tenant-ui/src/lib/api.ts`).
