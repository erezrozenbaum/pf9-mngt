...
- **History — N/A in Resource, Project, Domain columns** — Rewrote `v_comprehensive_changes` SQL view to use `COALESCE(history.name, live.name)` so deleted resources still show their name, project, and domain from the history snapshot instead of NULL. Also backfilled missing `project_name`/`domain_name` in `deletions_history` from `raw_json_snapshot`. Added `router` resource type to the view (was missing). Resource name completeness went from ~65% to ~99%.
