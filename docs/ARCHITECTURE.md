...
| **intelligence_worker** | pf9_db | TCP/PostgreSQL | Generate capacity/waste/risk/leakage/anomaly insights; harvest operational events from 10 source tables into `operational_events`; fire PSA webhooks | `POSTGRES_USER/PASSWORD` |
...
### Kubernetes Stack (v1.82.0+)

As of v1.82.0 every service ships as a fully tested Helm chart (`k8s/helm/pf9-mngt/`).
The Kubernetes topology mirrors the Docker Compose service graph but adds native health
probes, rolling-update strategies, and GitOps sync via ArgoCD.

```
                   ┌───────────────────────────────────┐
     Internet ───▶ │  nginx Ingress Controller  :443   │
                   └─────────────────┬─────────────────┘
                                     │
          ┌──────────────────────────┼──────────────────────────┐
          ▼                                                      ▼
  ┌──────────────┐                                      ┌──────────────┐
  │  pf9-ui      │  Deployment, 1+ replicas             │  pf9-api     │
  │  ClusterIP   │  image: pf9-mngt-ui                  │  ClusterIP   │
  │  :5173       │                                      │  :8000       │
  └──────────────┘                                      └──────┬───────┘
                                                               │
        ┌──────────────────────┬───────────────────────────────┤
 ...
 │  :5432     │         │  :6379     │                │  :389        │
 │  20 Gi PVC │         │  (cache)   │                │  5+1 Gi PVC  │
 └────────────┘         └────────────┘                └──────────────┘

  Workers (Deployments, 1 replica each, /tmp/alive liveness):
  ┌─────────────────┐ ┌──────────────────┐ ┌──────────────────┐
  │ backup-worker   │ │ ldap-sync-worker │ │ metering-worker  │
  │ 50 Gi PVC       │ │ (no PVC)         │ │ (no PVC)         │
  └─────────────────┘ └──────────────────┘ └──────────────────┘
  ┌─────────────────┐ ┌──────────────────┐ ┌──────────────────┐
  │notification-    │ │scheduler-worker  │ │search-worker     │
  │worker           │ │                  │ │                  │
  └─────────────────┘ └──────────────────┘ └──────────────────┘
  ┌─────────────────┐
  │snapshot-worker  │
  ...
  Job (pre-install / pre-upgrade hook):
  ┌────────────────────────────────────┐
  │  db-migrate — runs run_migration.py│
  │  blocks rollout until completed    │
  └────────────────────────────────────┘
...
**Secrets management:** All credentials are injected via Kubernetes Secrets (managed by
Bitnami Sealed Secrets). No credentials appear in `values.yaml`. See
[docs/KUBERNETES_GUIDE.md](KUBERNETES_GUIDE.md) for the complete deployment walkthrough.
...
### Scheduler image packaging hotfix (`scheduler_worker/Dockerfile`)
The scheduler container now includes the repository `shared/` package, fixing startup failures caused by `from shared.health_scoring import compute_security_posture_component` when running the worker in Kubernetes.
