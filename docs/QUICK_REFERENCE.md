...
- **CI: add `httpx` to integration test job** (v1.84.17 — NEW ✨): `test_tenant_portal_login_integration.py` imports `httpx`; missing from `pip install` in `.github/workflows/ci.yml` → `ModuleNotFoundError` at collection time → entire CI integration job aborted (exit 2). Added `httpx` to the install step.
