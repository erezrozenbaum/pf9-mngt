...
- **Snapshot coverage, restore, runbooks, metrics fixes** (v1.84.24 — NEW ✨): Snapshot coverage now counts `OK/(OK+ERROR)` only (SKIPPED excluded). Restore enabled via Helm flag. 9 admin-only runbooks hidden from tenant portal. Monitoring Current Usage now fetches from monitoring service HTTP API when file cache unavailable (K8s).
