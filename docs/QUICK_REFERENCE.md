...
- **Fix `tenant-ui` build: `api.ts` corruption** (v1.84.20 — NEW ✨): overlapping multi-replace interleaved code fragments into `api.ts` → `npm run build` failed with 10+ `TS1005`/`TS1160` TypeScript errors in CI. Rewrote file cleanly; `tsc --noEmit` passes.
