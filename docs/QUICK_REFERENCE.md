...
- **Metering Tab** (v1.15 + v1.15.1 Pricing ✨ | v1.94.5+ Comprehensive Chargeback ✨ | v1.95.2+ Advanced Billing System ⭐ | **v1.95.9–10 Metering Rebrand** 🆕):
  ...
  - **v1.95.12**: Security hardening — JWT fallback keys removed, infra IPs anonymized in docs/comments, bandit scan results untracked from git
...
- **Worker observability, LDAP conflict strategy, frontend resilience** (v1.83.52): New `GET /worker-metrics` Prometheus endpoint (all 5 workers, Redis heartbeats, stale-detection at 2× frequency); tenacity `@retry` on all worker DB connections; `ldap_sync_config.conflict_strategy` (`ldap_wins` / `local_wins`); `apiFetch` 30 s timeout + 3× GET retry + offline banner; real dashboard alert counts; CSS design tokens replacing hardcoded hex colours.
- **Bug fix: search_worker duplicate metrics + stacked retry** (v1.83.53): Duplicate `_report_worker_metrics()` block and globals reset run/error counters to zero on every module import; stacked `@retry` on `get_conn()` caused 9 DB reconnect attempts instead of 3. Removed duplicate block and excess decorator. `scheduler_worker` `executor.shutdown` changed to `wait=True` to prevent thread orphaning on SIGTERM.
- **Networks, SG rule editing, Dependency Graph, VM Provisioning** (v1.85.0): My Infrastructure screen adds Networks tab, Security Groups tab with inline add/delete rules, and SVG Dependency Graph (VM↔Net↔SG). New *New VM* screen (🚀): select flavor/image/network/SGs, create 1–10 VMs from tenant portal. 3 admin-only runbooks hidden (total 18 hidden, 7 visible). Monitoring HTTP fallback key fixed; snapshot coverage badge corrected.
- **Runbook execute, VM provisioning UX, graph expansion, activity log, dashboard** (v1.85.3 — NEW ✨): Execute button on runbook cards (parameter form, dry-run, history tab). Create VM: RFC-1123 name validation, fixed IP picker, cloud-init user/password. Dependency graph expanded to 5 node types + 4 edge types. VM list/CSV includes disk + IPs. Activity Log shows username + Keystone ID. Dashboard shows amber Skipped for skipped snapshot events.
...
- **Quota usage, runbook VM picker, monitoring host discovery** (v1.85.8): Dashboard Quota Usage bars now show real in-use figures (Nova/Cinder `?usage=true` fix). `vm_health_quickfix` + `snapshot_before_escalation` Execute dialogs now show the Target VM dropdown (`server_id` field detected via `x-lookup: vms`). Monitoring service auto-discovers hypervisor IPs from DB at startup when `PF9_HOSTS` is empty (new `/internal/prometheus-targets` endpoint).
- **Branding logo upload + monitoring docker-compose fixes** (v1.85.9): Admin Branding tab now has an Upload Image button + live preview (PNG/JPEG/GIF/WebP/SVG, max 512 KB; per-tenant via `?project_id=`). Fixed 3 docker-compose bugs that caused "No metrics collected yet": wrong `MONITORING_SERVICE_URL` DNS name, `PF9_HOSTS` defaulting to `localhost`, missing `monitoring/cache` volume on `tenant_portal`.
- **K8s CrashLoopBackOff hotfix** (v1.85.12 — NEW ✨): `pf9-tenant-ui` nginx hostname hardcoded as Docker Compose `tenant_portal` → K8s DNS fails at startup. Fixed via envsubst template; Helm sets `TENANT_PORTAL_UPSTREAM=pf9-tenant-portal:8010`. `pf9-monitoring` missing `httpx` in `requirements.txt` → `ModuleNotFoundError` at container startup. Added `httpx==0.27.2`.
- **SLA Compliance + Operational Intelligence Feed** (v1.86.0 — NEW ✨): Background `intelligence_worker` runs waste, risk, and SLA engines every 5 min writing to `operational_insights`. Waste engine: `waste_idle_vm` (7d/14d off), `waste_orphan_*`. Risk engine: `risk_health_decline`, `risk_snapshot_gap`; auto-creates `auto_incident` tickets. SLA engine: `sla_breach_warning`. `InsightsTab`: department workspace selector (4 tabs), severity badges, acknowledge/snooze/resolve, feed + aggregate views. 11 endpoints under `/api/intelligence/`. SLA compliance: `sla_commitments` + `sla_history` tables; `GET /api/sla/compliance/summary`; monthly PDF report.
- **Phase 2 Intelligence — Recommendations, Bulk Actions, Copilot Intents** (v1.88.0 — NEW ✨): Idle-VM insights generate actionable recommendations (cleanup runbook ≥14d, downsize ≥7d). Risk engine auto-creates support tickets. New bulk-acknowledge/bulk-resolve API endpoints. 5 Copilot intents (critical_insights, capacity_warnings, waste_insights, unacknowledged_insights_count, risk_summary). InsightsTab: SLA Summary sorted by breach status, Tenant/Project column, bulk-action bar, per-row recommendations panel.
- **SLA Summary route hotfix + Insights Feed Tenant column** (v1.88.1): `GET /api/sla/compliance/summary` was shadowed by `/{tenant_id}` route; fixed by reordering. Insights Feed adds Tenant/Project column from `metadata.project`.
- **Extended Forecasting, Cross-Region Intelligence & Anomaly Detection** (v1.89.0 — NEW ✨): Capacity engine extended: per-hypervisor compute (`capacity_compute`), per-project quota saturation (`capacity_quota_*`), confidence + r_squared metadata. Cross-region engine: imbalance / risk-concentration / growth-divergence insights. Anomaly engine: snapshot spike, VM-count spike, API error spike. New endpoints: `GET /api/intelligence/forecast`, `GET /api/intelligence/regions`. Intelligence Dashboard gains Capacity Forecast + Cross-Region tabs. DB: `idx_insights_type_prefix` (text_pattern_ops).
- **MSP Business Value Layer — Revenue Leakage, QBR, PSA Webhooks** (v1.90.0 — NEW ✨): Revenue Leakage engine: `leakage_overconsumption` + `leakage_ghost` detect upsell gaps vs contract entitlements. QBR PDF: `POST /api/intelligence/qbr/generate/{tenant_id}` with 6 configurable sections + labor-rate ROI. PSA outbound webhook: `GET/POST/PUT/DELETE /api/psa/configs`, per-config severity/type/region filters, Fernet-encrypted auth. Intelligence Settings panel (admin-only): labor rates, PSA CRUD, CSV entitlement import. Business Review button in Tenant Health. DB migration: 3 new tables (`msp_contract_entitlements`, `msp_labor_rates`, `psa_webhook_config`). 538 tests pass.
- **Client Health, Observer Role & Insights History** (v1.91.0 — NEW ✨): `portal_role` column on `tenant_portal_access` (`manager` | `observer`); observer tokens blocked at `require_manager_role()` from all 8 write routes. `GET /api/intelligence/client-health/{tenant_id}`: three-axis health (Efficiency, Stability, Capacity Runway). Tenant Health Overview screen with SVG circular dials. Observer invite magic-link flow. Insights History sub-tab. Operations summary bar. Sort by Tenant/Status.
- **SLA Commitment Editor + Compliance History** (v1.91.3): Tenant detail drawer SLA section: *Commitment* sub-tab (tier picker + manual KPI fields, upsert via `PUT /api/sla/commitments/{tenant_id}`). *History* sub-tab (12-month scorecard, breach red / at-risk amber per cell). SLA data fetches in parallel on panel open. 538 tests pass.
- **Monitoring push-cache, host network throughput, Copilot intents, dashboard polish** (v1.93.47 — NEW ✨): (1) Monitoring service now delivers metrics to the API after each scrape (Redis, 5 min TTL) — fixes live metrics N/A / allocation banner on Dashboard, Inventory, Tenant Portal, and Host Resource Metrics in Kubernetes. New internal endpoint `POST /internal/monitoring/push-cache` (authenticated by `X-Internal-Secret`). (2) `/monitoring/host-metrics` now merges live network throughput (`network_rx_mb`, `network_tx_mb`) and live CPU/memory/storage from the push-cache into hypervisor DB rows — fixes Host Resource Metrics Network Throughput always N/A. (3) Duplicate VM Provisioning card removed from Runbooks catalogue (filtered from DB-sourced list). (4) 8 new Copilot intents: `sla_compliance_status`, `active_alerts`, `migration_project_summary`, `restore_job_status`, `snapshot_policy_summary`, `intelligence_waste_summary`, `capacity_forecast`, `intelligence_risk_summary`; 4 new quick-suggestion categories. (5) Dashboard glassmorphism: dark-mode `.card` uses `backdrop-filter: blur(12px)` + gradient border; new `.card-glass`, `.gradient-header`, `.badge-pulse`, `.metric-value-animate` CSS utility classes.
- **Tenant Portal live metrics + API gateway + hostNetwork fix** (v1.93.46 — NEW ✨): Root cause of multiple "allocation-based usage" banners and N/A metrics confirmed and fixed. `hostNetwork: true` on the monitoring pod made its K8s Service endpoint resolve to the physical node IP; kube-proxy DNAT to a host-network endpoint fails cross-node — all pods on the same worker timed out. Fixed: (1) `hostNetwork` disabled — Flannel masquerade preserves hypervisor reachability; (2) tenant portal now routes metrics through the main API (`/internal/monitoring/vm-metrics`) instead of calling monitoring directly; (3) API→monitoring:8001 egress added to NetworkPolicy; (4) three wrong `MONITORING_SERVICE_URL` defaults (`pf9_monitoring` underscore → `pf9-monitoring` hyphen) fixed in `main.py` and `dashboards.py`.
- **Monitoring pod node placement — all metrics N/A** (v1.93.45): The monitoring pod rescheduled to `pf9-worker02` (10.0.0.12) which has no route to the hypervisor subnet (10.0.1.0/24), causing all Prometheus scrapes to time out. The cache stayed at `source: database` with storage/memory/network all `None`. Fixed by adding `nodeSelector: kubernetes.io/hostname: pf9-worker01` to the monitoring Helm deployment so the pod always lands on the node with the route.
- **Dashboard live metrics in K8s** (v1.93.44): `_load_metrics_cache()` in `dashboards.py` only searched for a local cache file (shared volume pattern). In Kubernetes the API pod and monitoring pod share no filesystem, so Dashboard VM Hotspots, Host Utilization, and Health Summary avg CPU/memory always showed DB allocation estimates instead of real Prometheus values. Fixed by adding an HTTP fallback to call `GET /metrics/vms` and `GET /metrics/hosts` on the monitoring service when no local file is found. Added `MONITORING_SERVICE_URL=http://pf9-monitoring:8001` env var to the API pod Helm deployment (already present on tenant-portal pod).
- **Live VM metrics, restore job cleanup, metering accuracy, docs syntax highlighting** (v1.93.43): (1) **Live VM metrics fixed** — monitoring pod was assigned a pod-CIDR IP (192.168.x.x) blocked by hypervisor firewalls; enabled `hostNetwork: true` on the K8s deployment so the pod uses the node IP (10.0.0.x), reaching libvirt-exporter on port 9177. Storage, memory, and network metrics now report real values instead of `None`. (2) **SSH + virsh fallback** — when `PF9_SSH_KEY_FILE` is configured, the monitoring service can collect VM metrics directly via `virsh domstats` over SSH, bypassing the exporter entirely. Set `PF9_SSH_USER` and `PF9_SSH_KEY_FILE` env vars. (3) **Restore job deletion** — `DELETE /restore/jobs/{job_id}` permanently removes non-active job records (PLANNED/FAILED/INTERRUPTED/CANCELED/SUCCEEDED). A ✕ Clear button appears per eligible row in the Restore Audit table. (4) **Stale restore job auto-timeout** — scheduler marks PLANNED jobs older than 2 h (`RESTORE_PLANNED_TIMEOUT_H`) and RUNNING/PENDING jobs older than 6 h (`RESTORE_RUNNING_TIMEOUT_H`) as FAILED, preventing orphaned jobs. (5) **Metering VM count accuracy** — `GET /metering/overview` now counts from the live `servers` table (excluding `DELETED`/`SOFT_DELETED`) instead of historical `metering_resources` rows, which inflated the count with deleted VMs. (6) **Docs tab syntax highlighting** — code blocks in the in-app documentation viewer are now highlighted using `highlight.js` with the `github-dark` theme via `marked-highlight`.
- **Security hardening: TLS warnings, backup checksums, readOnlyRootFilesystem, LDAP conn leaks, circuit breakers** (v1.93.21): H4 — `ldap_sync_worker` + `api/auth.py` emit `WARNING` when `verify_tls_cert=False`. H7 — backup worker computes SHA-256 after every `.sql.gz` write; stored in `backup_history.integrity_hash`; restore endpoint verifies hash before queuing (HTTP 409 on mismatch); `db/migrate_v1_93_21.sql` + `db/init.sql` updated. H8 — all 15 K8s Deployment templates get `readOnlyRootFilesystem: true` + `/tmp` emptyDir volumes. H10 — `api/auth.py` `authenticate()` + `_bind_external_ldap()` use `try/finally` to guarantee `conn.unbind_s()`. H15 — circuit-breaker wrapper (`_get_*_with_cb()`) added to all 9 workers; opens after 3 consecutive failures, backs off 60 s. 582+ tests pass, 0 xfailed.
- **Role-Based Dashboard Layer — Account Manager & Executive Views** (v1.92.0 — NEW ✨): Account Manager Dashboard (`My Portfolio` tab): per-tenant portfolio grid with SLA status badge, vCPU usage bar, critical/leakage counts, 6-card KPI strip. Powered by `GET /api/sla/portfolio/summary`. Executive Dashboard (`Portfolio Health` tab): fleet stacked SLA bar, 6 KPI cards (Fleet Health %, Breached, At Risk, Critical, Revenue Leakage/Month, Avg MTTR), narrative sections. Powered by `GET /api/sla/portfolio/executive-summary`. New `account_manager` + `executive` RBAC roles; two new departments with `default_nav_item_key` for persona auto-routing on login. `unit_price` column on `msp_contract_entitlements`. DB migration `migrate_v1_92_0_phase6.sql`. 538 tests pass.
- **Production nginx proxy + branding logo + `[object Object]` + Restore Center enhancements** (v1.85.11): `tenant-ui` nginx now proxies `/tenant/*` to backend (was missing → entire portal broken in prod). Branding logos (file-path in DB) now served as base64 data URLs. `apiFetch` unwraps FastAPI 422 array `detail` (fixes `[object Object]` error in admin UI). Restore Center: `MANUAL_IP` network/IP picker, result panel, email summary, expandable job history. Monitoring bootstrap runs unconditionally.
- **K8s branding/monitoring/runbook fixes** (v1.85.10): Branding save 422 fixed (logo URL validator accepts server-relative `/api/admin/…/branding-logo/` paths). Logo upload 400 fixed in K8s (extension fallback when nginx ingress strips part Content-Type). Monitoring empty-hosts bug fixed (`"".split(",")` → `[""]` → now `[]`). Monitoring startup race fixed (5× retry). `branding_logos` emptyDir volume added to K8s `pf9-api` Deployment. Runbooks now return `items_scanned` + `summary` (operators can see "Scanned 47 SGs; 0 violations" instead of just `0`). SQL injection B608 fixed in `capacity_forecast` engine.
...
- **Fix `tenant-ui` build: `api.ts` corruption** (v1.84.20): overlapping multi-replace interleaved code fragments into `api.ts` → `npm run build` failed with 10+ `TS1005`/`TS1160` TypeScript errors in CI. Rewrote file cleanly; `tsc --noEmit` passes.
- **Tenant portal crash-fix: `restore_jobs` region_id + `api.ts` adapter** (v1.84.19): `restore_jobs` table has no `region_id` column — 4 queries wrongly filtered by it → dashboard 500 `psycopg2.errors.UndefinedColumn`; all 16 `api.ts` functions rewritten as adapter layer — unwrap `{key:[...],total}` envelopes and remap field names to TypeScript interfaces → fixes `vms.filter is not a function` crash on all 7 tenant screens.
- **DB: `tenant_action_log` SELECT grant + K8s password sync** (v1.84.18): `tenant_portal_role` had `INSERT` but not `SELECT` on `tenant_action_log` → all post-login endpoints returned 500; K8s DB role password didn’t match the secret → login itself returned 500 in K8s. Applied `GRANT SELECT, INSERT` + `ALTER USER` fix.
- **CI: add `httpx` to integration test job** (v1.84.17): `test_tenant_portal_login_integration.py` imports `httpx`; missing from `pip install` in `.github/workflows/ci.yml` → `ModuleNotFoundError` at collection time → entire CI integration job aborted (exit 2). Added `httpx` to the install step.
- **Tenant Portal: Fix K8s NetworkPolicy + Keystone auth + MFA logic** (v1.84.16): NetworkPolicy `ingress.from` namespace `ingress-nginx` → `ingress-nginx-tenant`; added Keystone egress (TCP 443/5000); `Login.tsx` context-aware error messages; double `/v3` in Keystone auth URL stripped; system scope removed from token request (tenant users can't get system tokens); MFA `mfa_required` per-user flag is now authoritative (global `TENANT_MFA_MODE` is kill-switch only); K8s Helm `controlPlaneId: ""` → `"default"` + `os.getenv() or "default"` fallback; `GRANT SELECT ON role_assignments, projects TO tenant_portal_role` added to `init.sql` + migration; integration test suite T01–T10 added.
- **Tenant Portal: Fix 504 on login** (v1.84.15): `_keystone_auth()` was synchronous (`httpx.Client`) inside async route — blocks event loop under load → converted to `httpx.AsyncClient`; dev compose missing `VITE_TENANT_API_TARGET` → proxy hit `localhost:8010` inside container; K8s ingress missing `proxy-read-timeout` / `proxy-connect-timeout` annotations → added both to Helm tenant-ui ingress template.
- **Tenant Portal: Domain field on login + security hardening** (v1.84.14): added Keystone **Domain** field to tenant portal login form (default `Default`); `LoginRequest.domain` hardened with `max_length=255` + `^[A-Za-z0-9_.\-]{1,255}$` regex whitelist; security tests extended to S33 (S31–S33 cover domain validation).
- **Tenant Portal: Bug-fix & security hardening** (v1.84.13): fixed `log_auth_event` TypeError crash on grant/revoke; fixed Audit Log 500 (column aliases `details::text AS detail`, `timestamp AS created_at`); fixed batch grant transaction poisoning (savepoints per item); blocked `javascript:`/`data:` URIs in branding URL fields; added field length limits; security tests extended to S30.
- **Tenant Portal: Grant Access wizard + batch grant** (v1.84.12): 3-step wizard replaces raw Keystone User ID input — pick tenant (pill buttons), pick users (checkboxes w/ "Select all"), set MFA/notes. `PUT /access/batch` grants all selected users in one call. CP selector is now a dropdown (`GET /control-planes`). `GET /projects/{cp_id}` + `?project_id=` filter on `/users/{cp_id}`. Fixed invisible Grant Access button (`--pf9-accent` → `#2563eb`).
- **Tenant Portal: Friendly names on access rows** (v1.84.11): `user_name` + `tenant_name` columns added to `tenant_portal_access`; Grant Access form in Admin UI now has User Name and Tenant/Org Name fields; access table shows friendly labels with Keystone ID as secondary text.
- **Tenant Portal: Nav registration fix + guide corrections** (v1.84.10): `tenant_portal` tab now appears under Admin Tools in the UI (was missing from DB nav seed); `db/migrate_v1_84_9_nav.sql` applied to all environments; six guide inaccuracies corrected.
- **Tenant Portal: Branding + Admin Endpoints + Security Tests** (v1.84.9): Unauthenticated `GET /tenant/branding` (60 s cache, fail-safe defaults) in `tenant_portal/branding_routes.py`; `GET/PUT /api/admin/tenant-portal/branding/{cp_id}` (admin branding management, hex colour validation); `DELETE /api/admin/tenant-portal/mfa/{cp_id}/{keystone_user_id}` (admin MFA reset, full audit entry); `TenantPortalTab.tsx` admin UI with 4 sub-tabs (Access, Branding, Sessions, Audit); 27 P8 security tests (S01–S27) in `tests/test_tenant_portal_security.py`.
- **Tenant Portal web interface** (v1.84.4–v1.84.8): React + TypeScript SPA (`tenant-ui/`) with 7 screens; per-customer branding; MFA login. Nginx non-root crash fixed (v1.84.7). Dedicated `nginx-ingress-tenant` Helm controller on separate MetalLB IP (v1.84.8).
- **Tenant Portal P4/P4b/P4c/P4d** (v1.84.3 — NEW ✨): Self-service restore center (6 routes: GET restore-points, POST plan/execute, GET jobs/job-detail, POST cancel); full audit logging (`audit_helper.py`) on all 20 tenant endpoints using atomic `log_action()`; ops Slack/Teams webhook alert on restore execute + tenant email notification on complete/fail; 4 MFA routes (email OTP send, verify, TOTP setup, verify-setup with backup codes). Admin API extended with internal `POST /internal/tenant-restore/plan` + `/execute` endpoints validated by `X-Internal-Secret` header (`secrets.compare_digest`).
- **Tenant Portal read-only API** (v1.84.2): 14 `GET` endpoints across `environment_routes.py` (P3a: /tenant/vms, /tenant/vms/{id}, /tenant/volumes, /tenant/snapshots, /tenant/snapshots/{id}, /tenant/snapshot-history, /tenant/compliance, /tenant/dashboard, /tenant/events) and `metrics_routes.py` (P3b: /tenant/metrics/vms, /tenant/metrics/vms/{id}, /tenant/metrics/availability) plus runbooks (P3c: /tenant/runbooks, /tenant/runbooks/{name}). All routes use double-scoping: explicit `WHERE project_id=ANY AND region_id=ANY` plus RLS `inject_rls_vars()`. Ownership mismatches return 403 (never 404) to prevent existence oracle attacks.
- **Tenant Self-Service Portal Foundation** (v1.84.0 + v1.84.1): `tenant_portal_role` PostgreSQL role with minimal grants; RLS policies on 5 inventory tables (servers, volumes, snapshots, snapshot_records, restore_jobs); 5 new tables (`tenant_portal_access`, `tenant_portal_branding`, `tenant_action_log`, `tenant_portal_mfa`, `runbook_project_tags`) + `tenant_cp_view` (safely hides `password_enc`); isolated FastAPI service on port 8010 — JWT `role=tenant` (admin tokens explicitly rejected), Redis session binding with IP-change detection, MFA preauth flow, per-user rate limiting; 6 admin API endpoints (`GET /api/admin/tenant-portal/users/{cp_id}`, `GET/PUT /access/{cp_id}`, `DELETE/GET /sessions/{cp_id}`, `GET /audit/{cp_id}`); atomic Redis allow/block key sync on access changes; Helm deployment + NetworkPolicy (tenant portal blocked from reaching admin API, LDAP, SMTP); `docker-compose.yml` + `docker-compose.override.yml` updated.
...
- **RVTools K8s shared storage, file retention & per-dept default tab** (v1.82.18):
  ...
  - **DB migration** — `migrate_v1_82_18.sql`: `system_settings` table, `departments.default_nav_item_key`, idempotent nav item re-seed (fixes missing items on existing K8s), department back-fill.
- **LDAP Bootstrap & System Config tab** (v1.82.17):
  - **LDAP bootstrap** — `ensure_ldap_structure()` creates `ou=users` / `ou=groups` OUs and default admin in LDAP on startup. Fixes fresh K8s deployments where `create_user()` silently failed with `NO_SUCH_OBJECT`.
  - **System Config tab** — superadmin-only `⚙️` tab in Admin Tools: LDAP health card, PF9 Region card, SMTP card, inventory row counts. Setup Wizard banner when LDAP is unreachable.
- **External LDAP / AD Identity Federation** (v1.79.0 — NEW ✨): `ldap_sync_worker` Docker service syncs users from external LDAP/AD into pf9-mngt; group-to-role mapping with superadmin exclusion (DB CHECK); credential passthrough in `auth.py` — externally-synced users authenticate against origin directory; 10 new `/admin/ldap-sync` superadmin endpoints (CRUD, test, preview, manual trigger, logs); SSRF host validation; bind passwords Fernet-encrypted with `ldap_sync_key` Docker secret; `crypto_helper.py` shared encrypt/decrypt; `db/migrate_ldap_sync.sql` adds 3 tables + 3 `user_roles` columns; session revocation on deactivation
- **Security & Auth Hardening** (v1.78.0): LDAP DN injection closed (`ldap.dn.escape_dn_chars()` on all 4 DN-construction sites); LDAP network timeout (5 s) on all 7 `ldap.initialize()` call sites; `verify_admin_credentials` raises HTTP 503 when unconfigured (was silently bypassing auth) + `hmac.compare_digest()` to prevent timing attacks; `datetime.utcnow()` replaced with `datetime.now(timezone.utc)` in all auth paths; `rbac_middleware` caches `TokenData` in `request.state` (eliminates double `verify_token()` DB call per request); `SMTP_PASSWORD` now resolved via `read_secret()` consistent with all other credentials
...
- **Database Integration**: PostgreSQL 16 with 95+ tables for historical tracking + metering + departments/navigation + runbooks + onboarding + tenant portal RLS
