...
- **Tenant Portal: Domain field on login + security hardening** (v1.84.14 — NEW ✨): added Keystone **Domain** field to tenant portal login form (default `Default`); `LoginRequest.domain` hardened with `max_length=255` + `^[A-Za-z0-9_.\-]{1,255}$` regex whitelist; security tests extended to S33 (S31–S33 cover domain validation).
