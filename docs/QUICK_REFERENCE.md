...
- **Tenant Portal: Nav registration fix + guide corrections** (v1.84.10 — NEW ✨): `tenant_portal` tab now appears under Admin Tools in the UI (was missing from DB nav seed); `db/migrate_v1_84_9_nav.sql` applied to all environments; six guide inaccuracies corrected.
