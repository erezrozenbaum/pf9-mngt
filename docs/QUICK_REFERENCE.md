...
- **Fix `tenant-ui` build: `api.ts` duplicate content truncated** (v1.84.21 — NEW ✨): v1.84.20 replacement appended clean content after corrupted content → 1341 lines, two copies, Docker build failed again. Truncated to first 661 lines; `docker compose build tenant_ui` passes.
