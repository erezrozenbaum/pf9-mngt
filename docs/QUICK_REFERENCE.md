...
- **Tenant Portal live metrics + API gateway + hostNetwork fix** (v1.93.46 — NEW ✨): Root cause of multiple "allocation-based usage" banners and N/A metrics confirmed and fixed. `hostNetwork: true` on the monitoring pod made its K8s Service endpoint resolve to the physical node IP `172.17.30.164`; kube-proxy DNAT to a host-network endpoint fails cross-node — all pods on `pf9-worker02` timed out. Fixed: (1) `hostNetwork` disabled — Flannel masquerade preserves hypervisor reachability; (2) tenant portal now routes metrics through the main API (`/internal/monitoring/vm-metrics`) instead of calling monitoring directly; (3) API→monitoring:8001 egress added to NetworkPolicy; (4) three wrong `MONITORING_SERVICE_URL` defaults (`pf9_monitoring` underscore → `pf9-monitoring` hyphen) fixed in `main.py` and `dashboards.py`.
