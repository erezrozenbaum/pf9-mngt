...
- **Tenant Portal: Friendly names on access rows** (v1.84.11 — NEW ✨): `user_name` + `tenant_name` columns added to `tenant_portal_access`; Grant Access form in Admin UI now has User Name and Tenant/Org Name fields; access table shows friendly labels with Keystone ID as secondary text.
