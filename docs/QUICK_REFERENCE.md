...
- **Tenant portal data fixes + REPLACE restore mode** (v1.84.22 — NEW ✨): 6 data display bugs fixed (status `OK`/`ERROR`, vCPUs from flavors JOIN, IPs from JSONB, restore-points via `server_id`, runbooks visibility, calendar normalisation). Restore Center adds Replace In-Place mode: mandatory safety snapshot + VM-name confirmation.
