...
- **Inventory sync bar + pre-restore snapshot + 15-min RVTools interval** (v1.84.23 — NEW ✨): Shell header shows "Inventory synced X min ago" + Refresh button. NEW mode restore now has optional pre-restore snapshot checkbox. Critical `tenant_portal_role` DB grants fixed (flavors + inventory_runs). RVTools interval: 60 → 15 min.
