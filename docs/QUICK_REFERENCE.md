...
- **DB: `tenant_action_log` SELECT grant + K8s password sync** (v1.84.18 — NEW ✨): `tenant_portal_role` had `INSERT` but not `SELECT` on `tenant_action_log` → all post-login endpoints returned 500; K8s DB role password didn’t match the secret → login itself returned 500 in K8s. Applied `GRANT SELECT, INSERT` + `ALTER USER` fix.
