...
- **Metering Worker Crash Fix** (v1.74.6 — NEW ✨): Phase 5B migration guard hardened to require all 6 target `region_id` columns before skipping; `security_groups.region_id` column + index added to `init.sql` / `migrate_multicluster.sql`; fixes metering_worker crash on `collect_quota_usage` LATERAL subquery
