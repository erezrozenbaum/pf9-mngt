...
- **Networks, SG rule editing, Dependency Graph, VM Provisioning** (v1.85.0 — NEW ✨): My Infrastructure screen adds Networks tab, Security Groups tab with inline add/delete rules, and SVG Dependency Graph (VM↔Net↔SG). New *New VM* screen (🚀): select flavor/image/network/SGs, create 1–10 VMs from tenant portal. 3 admin-only runbooks hidden (total 18 hidden, 7 visible). Monitoring HTTP fallback key fixed; snapshot coverage badge corrected.
