...
- **Tenant Portal: Grant Access wizard + batch grant** (v1.84.12 — NEW ✨): 3-step wizard replaces raw Keystone User ID input — pick tenant (pill buttons), pick users (checkboxes w/ "Select all"), set MFA/notes. `PUT /access/batch` grants all selected users in one call. CP selector is now a dropdown (`GET /control-planes`). `GET /projects/{cp_id}` + `?project_id=` filter on `/users/{cp_id}`. Fixed invisible Grant Access button (`--pf9-accent` → `#2563eb`).
