...
> **v2.12.3 FIX**: Node Logs HTTP 404 on log fetch — `_fetch_via_resmgr()` was passed the PostgreSQL integer `id` (e.g. `6`) instead of the PF9 resmgr UUID (`raw_json->'service'->>'host'`). `_get_nodes()` now returns `resmgr_id`; `get_node_logs()` uses it. All four KVM nodes now return log lines.
> **v2.12.2 FIX**: Node Logs dropdown empty — `_get_nodes()` queried non-existent column `hypervisor_hostname`; corrected to `hostname`. System Settings 403 — `require_permission("admin","read")` nested DB call failed under PgBouncer transaction-pool mode; replaced with `require_authentication` + inline role check. Blank right panel on Node Logs/System Settings — both tabs added to `hideDetailsPanel` list. `role_permissions` seed updated with `admin` resource rows for `superadmin` and `admin` roles.
> **v2.12.1 FIX**: Responsive `PercentBar` layout in Platform Health (`flex: 1; minWidth: 28px` replaces fixed 100 px bar — no card overflow in narrow viewports); network sparkline wrapped in `overflowX: hidden`; `nav_items` DB rows for `node_logs`, `admin_settings`, `clea_policies` now seeded via migration (Admin Tools grouped nav shows all 3 tabs); CI flake8 F824 fix (`global _read_pool` removed from `get_read_connection()`); TypeScript fixes in `App.tsx` and `NodeLogsTab.tsx` (orphaned fragment, unused import, Omit spread).
...
> **v2.7.0 NEW**: Event Bus (`emit_event()` fire-and-forget writer to `operational_events`); Platform Health endpoint (`GET /api/admin/platform/health` — DB latency, Redis ping, pool stats, worker last-run); Platform Health UI tab under Admin Tools.
> **v2.6.8 FIX**: Drift detection false positive on new-VM provisioning — `_detect_drift()` in `db_writer.py` now skips events where `old_value IS NULL`; first-time field assignments (e.g. boot volume `server_id: NULL → vm_uuid`) no longer generate drift events. Only genuine state changes between two known non-null values emit drift events.
...
- **Top Host Utilization**: Real-time CPU/memory usage across compute nodes (live Prometheus data via monitoring service HTTP API in K8s — v1.93.44; monitoring pod pinned to correct node v1.93.45; cross-node connectivity fixed v1.93.46)
