...
> **Version**: v2.12.7 — Node Logs `Invalid Date` fix: `fmtTs()` helper in `NodeLogsTab.tsx` extracts `HH:MM:SS` directly from PF9 timestamp strings; dependency graph `aggregate` node type added — hypervisors with zero running VMs now connect to their host aggregates.
> Previous: v2.12.6 — Node log timestamp parsing fix: `_parse_raw_log()` now recognises PF9's Python logging format (`YYYY-MM-DD HH:MM:SS,mmm - module.py LEVEL - message`); `ts` field is now populated on all hostagent/ostackhost log lines.
