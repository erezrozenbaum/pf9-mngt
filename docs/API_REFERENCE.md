...
> **Version**: v2.16.3 — Scheduler-worker NetworkPolicy now allows egress to PF9 hypervisor exporter ports `9177`/`9388`, restoring live host/VM metrics polling in Kubernetes; metrics collector logs now emit explicit timeout messages and exception class details for faster diagnostics.
> Previous: v2.16.2 — Drift auto-ticketing now suppresses expected policy-driven changes using rule-based filters (`drift.auto_ticket.ignore_rules` in `system_settings`) with a default snapshot auto-delete suppression for `auto-*` resources; inventory tabs now hide the unused right-side details panel for a cleaner full-width layout.
