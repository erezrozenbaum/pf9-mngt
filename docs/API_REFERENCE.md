...
> **Version**: v2.16.1 — Portfolio Health and My Portfolio dashboards add SLA Defense open-alert drill-down tables with in-line `Resolve` / `Dismiss` actions, explicit zero-alert visibility messaging, and improved SLA Fleet Health legend readability; portfolio tabs also suppress the unused right details panel.
> Previous: v2.16.0 — Proactive SLA defense engine generates `sla_defense_alerts` from active SLA commitments plus open capacity/risk/anomaly insights; new admin endpoints under `/api/admin/sla/defense` support list/summary/dismiss/resolve flows; executive and account-manager dashboards now surface open SLA defense alerts.
