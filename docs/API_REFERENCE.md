...
> **Version**: v2.16.2 — Drift auto-ticketing now suppresses expected policy-driven changes using rule-based filters (`drift.auto_ticket.ignore_rules` in `system_settings`) with a default snapshot auto-delete suppression for `auto-*` resources; inventory tabs now hide the unused right-side details panel for a cleaner full-width layout.
> Previous: v2.16.1 — Portfolio Health and My Portfolio dashboards add SLA Defense open-alert drill-down tables with in-line `Resolve` / `Dismiss` actions, explicit zero-alert visibility messaging, and improved SLA Fleet Health legend readability; portfolio tabs also suppress the unused right details panel.
