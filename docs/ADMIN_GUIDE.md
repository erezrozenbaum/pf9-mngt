...
| Run rightsizing analysis | UI → Runbooks → vm_rightsizing (dry-run) | Operator+ |
| Check capacity forecast | UI → Runbooks → capacity_forecast | Operator+ |
...
### v2.16.3 — Scheduler metrics egress fix and clearer collector errors

- **Scheduler worker metrics egress restored** (`k8s/helm/pf9-mngt/templates/network-policies.yaml`): Added explicit egress allow rules on TCP `9177` and `9388` so scheduler metrics polling can reach PF9 hypervisor libvirt/node exporters in Kubernetes.
- **Actionable collector failure logs** (`host_metrics_collector.py`): Exporter poll failures now distinguish timeouts (`timeout after 10s`) and include exception class names for non-timeout errors, improving triage for network or exporter-side issues.

### v2.16.2 — Drift ticket suppression tuning and inventory layout cleanup

- **Expected drift suppression for auto-ticketing** (`db_writer.py`): Drift auto-ticket creation now checks ignore rules before creating a support ticket. Defaults include suppression for expected retention automation behavior (snapshot status changes to `deleted` for names matching `auto-*`).
- **Configurable ignore rules** (`system_settings`): Operators can define JSON rules under `drift.auto_ticket.ignore_rules` to suppress known-benign drift patterns (by severity/resource/field/value/description/resource name regex) without code changes.
- **Inventory full-width layout** (`pf9-ui/src/App.tsx`): The right details pane is now hidden on inventory-focused tabs (`projects`, `ports`, `images`, `flavors`, `keypairs`, `aggregates`, `volume_types`, `server_groups`, `quotas`) to reduce dead space and improve readability.
...
- **Schema / migration**: Apply `db/migrate_v2_16_0_sla_defense.sql` on existing environments. The migration creates `sla_defense_alerts` plus indexes for open-alert uniqueness, status/timestamp sorting, and project filtering. Fresh installs already get the table from `db/init.sql`.
- **Admin API**: New endpoints under `/api/admin/sla/defense` allow operators to list alerts, fetch the open warning/critical summary used by the dashboards, and mark alerts as `dismissed` or `resolved` with an optional note.
- **Dashboard visibility**: Both the Executive Dashboard and the Account Manager Dashboard now show a compact SLA Defense KPI so operators can see open proactive alerts without navigating away from portfolio views.

### v2.15.0 — Smart query LLM fallback + realtime anomaly quick-check

- **Smart query semantic fallback** (`api/smart_queries.py`, `api/search.py`): `GET /api/search/smart` still tries regex templates first, but now falls back to a lightweight LLM classifier when no regex matches. The classifier receives the question plus available query IDs/descriptions and must return one query ID or `null`. Safeguards: fallback only runs when `COPILOT_ENABLED=true`, requires a configured non-`builtin` backend, and has a strict 2-second timeout. Response payload now includes `matched_via` (`regex` or `llm`) for auditability.
- **Realtime anomaly fast path** (`api/event_bus.py`): After writing an operational event, the event bus now performs a quick anomaly check for `vm.cpu_spike`, `vm.ram_spike`, and `quota.sudden_jump`. It reads Redis baseline stats from `pf9:stats:{entity_type}:{entity_id}`, compares current metric value against `mean ± 3σ`, upserts a realtime anomaly insight, and emits an `anomaly.realtime` event. DB writes are bounded with `SET LOCAL statement_timeout = '200ms'` to keep request-path impact negligible. Toggle: `REALTIME_ANOMALY_ENABLED` (default `true`).
...
- **Branding URL validation** — `tenant-ui` now restricts logo and favicon URLs to `http:`, `https:`, or relative paths. `data:` and `javascript:` scheme URLs are rejected, protecting against content-injection attacks.
- **Password field autocomplete** — All password and username inputs across the management UI and tenant portal now carry proper `autocomplete` attributes (`username`, `current-password`, `new-password`). Improves password-manager compatibility and resolves browser security warnings.
- **Docker base images pinned to exact patch versions** — All 15 Dockerfiles now reference exact image digests (`python:3.11.12-slim`, `postgres:16.8-alpine`, `redis:7.4.3-alpine`, `node:20.19.1-alpine`, `node:22.14.0-alpine`, `nginx:1.27.5-alpine`). Prevents unintended regressions from floating tags.
- **Optional pre-migration database backup** — `run_migration.py` now checks for `PRE_MIGRATION_BACKUP=true`. When set, it runs `pg_dump | gzip` to `PF9_BACKUP_PATH` before applying any pending migration. The backup is owner-readable only (chmod 0600). A failure emits a warning and allows migration to proceed — the guard is non-blocking.
- **Migration rollback guidance** — `docs/DEPLOYMENT_GUIDE.md` now documents the policy for down-migration scripts (`migrate_*_down.sql`) and the manual procedure to apply them.
- **Prometheus alerting rules** — The Helm chart now deploys a `PrometheusRule` CRD (`pf9-mngt-alerts`) into the `pf9-mngt` namespace. Rules cover pod crash-looping, deployment unavailability, API latency (p99 > 2 s), database connection pool exhaustion, and worker heartbeat loss. Gated by `alerting.enabled: true` in `values.yaml`.
- **Loki + Promtail log aggregation** — `k8s/monitoring/loki-values.yaml` configures Loki (10 Gi PVC, 15-day retention) and Promtail (ships all pod logs with namespace/pod/container labels). Install via `helm install loki grafana/loki-stack -n monitoring -f k8s/monitoring/loki-values.yaml`. See [KUBERNETES_GUIDE.md](KUBERNETES_GUIDE.md) §11 for full setup.

### v1.93.28 — Code hardening: timeouts, chmod, SHA256, template validation, token cleanup, nginx rate limit (✅ Complete)

- **Configurable process timeouts** — Previously hardcoded values in `backup_worker/main.py` and `scheduler_worker/main.py` now load from env vars: `PF9_BACKUP_DUMP_TIMEOUT_SEC`, `PF9_RESTORE_TIMEOUT_SEC`, `PF9_BACKUP_VALIDATE_TIMEOUT_SEC`, `PF9_LDAP_EXPORT_TIMEOUT_SEC`, `PF9_LDAP_RESTORE_TIMEOUT_SEC`, `PF9_RVTOOLS_TIMEOUT_SEC`.
- **Backup file permissions restricted to owner-only** — `os.chmod(filepath, 0o600)` called immediately after each DB and LDAP backup write in `backup_worker/main.py`. Prevents world-readable backups if process umask is permissive.
- **SHA256 in cache keys** — `api/cache.py` now uses `hashlib.sha256(...).hexdigest()[:32]` instead of MD5. No functional change; eliminates Bandit B324 warnings.
- **Notification worker fails fast on missing template directory** — `notifications/main.py` raises `RuntimeError` immediately if the Jinja2 template directory is missing, rather than failing on first email send.
- **Expired reset token cleanup** — `scheduler_worker/main.py` purges expired rows from `password_reset_tokens` on each RVTools maintenance cycle via `_cleanup_expired_tokens()`.
- **Dev nginx rate limiting** — `nginx/nginx.conf` (dev) now has `limit_req_zone` zones and `limit_req` on `/api/` (20 r/s, burst 40) and `/auth/` (5 r/m, burst 10) — matching prod behaviour and catching rate-limit issues in development.

### v1.93.27 — K8s hardening: ResourceQuota, PDB, HPA, imagePullPolicy (✅ Complete)

- **Namespace resource quota** — `pf9-mngt-quota` created in namespace: `requests.cpu=3000m`, `requests.memory=6Gi`, `limits.cpu=20000m`, `limits.memory=20Gi`, `pods=40`. Gated by `resourceQuota.enabled`.
- **PodDisruptionBudgets** — PDBs added for `pf9-api`, `pf9-tenant-portal`, `pf9-monitoring` (`minAvailable: 1`). Prevents complete service outage during node drains. Gated by `podDisruptionBudget.enabled`.
- **HPA scaffold** — HorizontalPodAutoscaler scaffold for API (CPU/memory 80% target, max 5 replicas). Disabled by default; enable by setting `hpa.enabled: true` after verifying `metrics-server` is installed.
- **imagePullPolicy: Always** — Ensures all app pods always pull the latest image on restart. Changed from `IfNotPresent`.
...
- **VM Dependency Annotation**: Operators annotate ordering constraints between VMs ("web-tier cannot start until db-vm is migrated"). Circular dependency check enforced at API level. Shown as dependency badges in the expanded VM row.
- **Per-VM Migration Status**: New `migration_status` column with six values: `not_started`, `assigned`, `in_progress`, `migrated`, `failed`, `skipped`. Colour-coded pill column in the VM table. Single and bulk update endpoints.
- **Per-VM Mode Override**: Operators can force `warm` or `cold` on any VM regardless of engine classification. 🔒 icon when override is active.
- **Tenant Migration Priority**: Integer priority field (lower = earlier) on each tenant. Editable column in the Tenants tab. Used by cohort auto-assign "by priority" strategy.
- **Per-Tenant Readiness Checks**: 5 auto-derived checks per tenant — `target_mapped`, `network_mapped`, `quota_sufficient`, `no_critical_gaps`, `vms_classified`. 🟢/🟡/🔴 in Tenants tab.
- **🔍 Find & Replace (Tenants)**: Bulk find-and-replace across any of the 4 target name/description fields (`target_domain_name`, `target_domain_description`, `target_project_name`, `target_display_name`). Literal substring, case-insensitive default, preview diff before applying. Affected rows reset to unconfirmed for review. `POST /projects/{id}/tenants/bulk-replace-target`.
- **🔍 Find & Replace (Network Map)**: Same capability for `target_network_name` across all network mappings. `POST /projects/{id}/network-mappings/bulk-replace`.
- **✓ Confirm All**: One-click bulk-confirm buttons in both Tenants and Network Map toolbars. `POST /projects/{id}/tenants/confirm-all` and `/network-mappings/confirm-all`.
- **Domain & Project Descriptions**: Tenants table gains **Domain Desc.** (`target_domain_description`) and **Proj. Desc.** (`target_display_name`) columns — matching PCD's Domain and Project description fields. Both are auto-seeded from the source name on first upload and editable inline. DB migration: `db/migrate_descriptions.sql`.
- **Data Hygiene**: Stale network mappings for networks no longer present in a re-uploaded RVTools file are automatically purged. "Clear RVTools Data" also clears all network mappings.
- **DB Migration**: `db/migrate_cohorts_and_foundations.sql`, `db/migrate_target_preseeding.sql`, `db/migrate_cohort_fixes.sql`, `db/migrate_descriptions.sql`.
...
- **40+ Built-in Intents**: Inventory counts, VM power states (powered on/off/error), capacity metrics (CPU/memory/storage), down hosts, networking (networks, subnets, routers, floating IPs), snapshots, drift, compliance, metering, users, role assignments, activity logs, runbooks, backups, notifications, security groups, and full infrastructure overview — powered by live SQL queries and Platform9 API calls.
- **Live Quota Lookups**: Three separate quota/usage intents — *"quota of tenant X"* fetches configured limits from Platform9 (Compute, Storage, Network), *"usage for tenant X"* shows actual resource consumption from the local DB, *"quota and usage for tenant X"* shows a side-by-side comparison table with % utilization and color-coded indicators (🟢🟡🔴). Requires `PF9_AUTH_URL`, `PF9_USERNAME`, `PF9_PASSWORD` environment variables.
...
- **Windows Glance property auto-patch** (v1.44.2): before creating the boot volume the execution thread patches the Glance image with `os_type=windows`, `hw_disk_bus=scsi`, `hw_scsi_model=virtio-scsi`, `hw_firmware_type=bios`. This ensures Nova configures the correct virtual hardware (virtio-scsi bus, BIOS firmware) regardless of whether the image was uploaded with those properties set. The patch is idempotent and a failure is logged as `image_patch_warning` in the activity log without blocking provisioning.
