...
if ($allGood) {
    ...
    if ($isDemoMode) {
        ...
        $metricsInterval = $envCheckMap['METRICS_INTERVAL_SECONDS']
        if (-not $metricsInterval) { $metricsInterval = '60' }
        Write-Host "  Interval : $metricsInterval s" -ForegroundColor Gray
        ...
        if ($rvInterval -and [int]$rvInterval -gt 0) {
            ...
            $scheduleTime = $envCheckMap['RVTOOLS_SCHEDULE_TIME']
            if (-not $scheduleTime) { $scheduleTime = '03:00' }
            Write-Host "  Inventory: daily at $scheduleTime UTC (RVTOOLS_SCHEDULE_TIME)" -ForegroundColor Gray
        }
    }
