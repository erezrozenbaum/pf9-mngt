...
# Canonical column names we want  Γזע  list of aliases that RVTools versions use
...
def build_column_map(sheet_headers: List[str], prefix: str = "") -> Dict[str, int]:
    ...
    canonical name Γזע column index.  Uses fuzzy matching via COLUMN_ALIASES.
...
def extract_row(row_values: list, col_map: Dict[str, int]) -> Dict[str, Any]:
    """
    Extract a dict of canonical-name Γזע cell-value from a single Excel row.
    """
    out: Dict[str, Any] = {}
...
def detect_tenant_folder(vm: Dict[str, Any], depth: int = 2) -> Optional[str]:
    ...
    E.g. "/DatacenterX/vm/TenantA/Production" at depth=2 Γזע "TenantA"
    """
...
def detect_tenant_resource_pool(vm: Dict[str, Any]) -> Optional[str]:
    ...
    E.g. '/Datacenter1/vCloud1_Pvdc1/Resources/OrgName_vDC_123 (uuid)' Γזע 'OrgName_vDC_123'
    """
...
def _strip_uuid_suffix(segment: str) -> str:
    ...
    E.g.  'shaal_aviv (a1b3000a-80a6-42eb-a04d-393c54e009d2)' Γזע 'shaal_aviv'
    """
...
def detect_tenant_vcd_folder(vm: Dict[str, Any]) -> Optional[Tuple[str, str]]:
    ...
    Typical folder paths:
      /Datacenter1/vCD-Node1/OrgName (uuid)/OrgName-VDC-NNNNNN (uuid)/Γאª
    ...
    # ΓפאΓפא Try folder_path first ΓפאΓפא
    folder = str(vm.get("folder_path", "")).strip().strip("/")
    ...
    # ΓפאΓפא Try resource_pool ΓפאΓפא
    rp = str(vm.get("resource_pool", "")).strip().strip("/")
    ...
    # ΓפאΓפא Strategy 2: UUID-bearing segments (vCD managed) ΓפאΓפא
...
def detect_tenant_annotation(vm: Dict[str, Any], field_name: str = "Tenant") -> Optional[str]:
    ...
    E.g. annotation="Tenant: Acme Corp; Env: Prod" Γזע "Acme Corp"
    """
...
def classify_network_type(network_name: str) -> str:
    ...
    Classify a network based on naming conventions:
      - 'vlan_based'  Γאפ external VLAN-backed portgroup
      - 'nsx_t'       Γאפ NSX-T overlay segment (not supported on PCD)
      - 'isolated'    Γאפ isolated / internal-only network
      - 'standard'    Γאפ standard vSwitch / unclassified
    """
...
def classify_migration_mode(vm: Dict[str, Any], rules: Dict[str, Any]) -> MigrationModeResult:
    ...
    if power in ("poweredoff", "powered off", "suspended"):
        reasons.append(f"VM is {power} Γאפ cold migration only")
        return MigrationModeResult(mode="cold_required", reasons=reasons)
    ...
    if cbt is False or cbt == "":
        reasons.append("CBT (Changed Block Tracking) not enabled Γאפ warm migration less efficient")
    ...
    very_large = rules.get("disk_very_large_threshold_gb", 5000)
    if total_disk_gb >= very_large:
        reasons.append(f"Very large disk ({total_disk_gb:.0f} GB) Γאפ long pre-copy phase")
        risky = True

    snap_count = int(vm.get("snapshot_count", 0) or 0)
    if snap_count >= rules.get("snapshot_depth_critical", 5):
        reasons.append(f"Deep snapshot chain ({snap_count}) Γאפ consolidate before warm migration")
        risky = True
...
def recommend_agent_sizing(
    ...
    if target_vms_day > 0:
        # User specified target VMs/day Γזע derive agents needed
        ...
        reasoning.append(f"Target: {target_vms_day} VMs/day Γזע ~{vms_per_agent_per_day:.0f} VMs/agent/day Γזע {recommended} agents")

    elif duration_days > 0:
        # User specified project duration Γזע derive agents needed
        ...
        reasoning.append(
            ...
            f"({working_days_week}d/wk ├ק {working_hours}h/d = {effective_days:.0f} effective days)"
        )
        reasoning.append(f"Need {vms_per_day_needed:.1f} VMs/day Γזע {recommended} agents")
    else:
...
def estimate_vm_time(
    ...
    Warm migration (vJailbreak two-phase):
      ...
      Incremental: Daily delta sync = in_use ├ק daily_change_rate (VM stays up)
    ...
    # Effective bandwidth: Mbps Γזע GB/h (with real-world factors)
...
def generate_migration_plan(
    ...
    # ΓפאΓפא Throughput model for daily scheduling ΓפאΓפא
    ...
    # Build daily schedule Γאפ process cohorts sequentially, each cohort starts on a fresh day
    # Group VMs by cohort (already sorted by cohort_order Γזע tenant Γזע priority Γזע disk)
...
def compute_quota_requirements(
    ...
    for t in tenants:
        ...
        if cpu_ratio >= 6.0 and vcpu > 200:
            warnings.append(f"High CPU density ({cpu_ratio}:1) with {vcpu} vCPUs Γאפ monitor for contention")
        if ram_ratio >= 1.8:
            warnings.append(f"RAM overcommit {ram_ratio}:1 Γאפ ensure workloads are not memory-intensive")
        if disk_gb_rec > 50_000:
            warnings.append(f"Recommended disk quota {disk_gb_rec/1024:.1f} TB Γאפ verify PCD storage capacity")
...
def compute_node_sizing(
    ...
    max_util_pct: float = 70.0,         # hard ceiling Γאף HA headroom lives inside this
    ...
    # ΓפאΓפא Per-node capacity ΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפא
    ...
    # ΓפאΓפא Safe capacity of the CURRENT cluster ΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפא
    safe_vcpu_current = cluster_vcpu_total * util_frac
    safe_ram_current  = cluster_ram_total  * util_frac

    # ΓפאΓפא Available headroom (safe capacity minus what is already running)
    headroom_vcpu = max(0.0, safe_vcpu_current - already_used_vcpu)
    headroom_ram  = max(0.0, safe_ram_current  - already_used_ram)

    # ΓפאΓפא Migration workload with peak buffer ΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפא
    ...
    # ΓפאΓפא Deficit = how much workload the current cluster CANNOT absorb Γפא
    deficit_vcpu = max(0.0, demand_vcpu - headroom_vcpu)
    deficit_ram  = max(0.0, demand_ram  - headroom_ram)

    # ΓפאΓפא Additional nodes to cover the deficit ΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפא
    ...
    # ΓפאΓפא Post-migration utilisation ΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפא
    ...
    # ΓפאΓפא Explanation steps for UI display ΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפא
    steps = [
        ...
        f"Migration workload (├ק{peak_frac:.2f} peak buffer): {demand_vcpu:.0f} vCPU | {demand_ram:.0f} GB",
        f"Deficit: {deficit_vcpu:.0f} vCPU | {deficit_ram:.0f} GB  Γזע  add {nodes_to_add} node(s) (binding: {binding_dimension})",
        f"Post-migration utilisation: CPU {post_cpu_pct}% | RAM {post_ram_pct}%  (target Γיñ {max_util_pct:.0f}%)",
    ]
    ...
    if disk_tb_required > 0:
        warnings.append(f"Cinder storage required: {disk_tb_required} TB Γאפ provision independently (Ceph/SAN/NFS)")
...
def analyze_pcd_gaps(
    ...
    # ΓפאΓפא Flavors: find (vcpu, ram_mb) combos with no matching PCD flavor ΓפאΓפאΓפאΓפאΓפא
    ...
    # ΓפאΓפא Networks: source VM networks not present in PCD ΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפא
    ...
    # ΓפאΓפא Images: OS families needed ΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפאΓפא
    ...
    # ΓפאΓפא Tenant mapping gaps: tenants with no target domain/project set ΓפאΓפאΓפאΓפאΓפאΓפאΓפא
    for t in tenants:
        if not t.get("include_in_plan", True):
            continue
...
# =====================================================================
# 9.  Phase 3.0 Γאפ Tenant Ease Score & Auto-Assign Cohorts
# =====================================================================

# Default weights Γאפ must sum to 100
...
def _norm(value: float, max_val: float) -> float:
    """Normalise a value to 0Γאף1.  Returns 0 if max_val is 0."""
    if max_val <= 0:
        return 0.0
...
def compute_tenant_ease_scores(
    ...
    """
    Compute a 0Γאף100 ease score for each tenant.

    Lower score  Γƒ╣  easier (migrate first: small, low-risk, fully-supported).
    Higher score Γƒ╣  harder (save for later: large, risky, unsupported OS).

    Each input tenant dict should contain:
        tenant_id, tenant_name
        vm_count               (int)
        total_used_gb          (float)   Γאפ sum(in_use_gb) across VMs
        avg_risk_score         (float)   Γאפ average risk score 0Γאף100
        os_support_rate        (float)   Γאפ fraction 0Γאף1 of VMs with supported OS
        distinct_network_count (int)
        cross_tenant_dep_count (int)     Γאפ dependency edges pointing outside this tenant
        cold_vm_count          (int)     Γאפ VMs classified cold
        unconfirmed_count      (int)     Γאפ unconfirmed target+network mappings
        total_vm_or_mapping    (int)     Γאפ denominator for unconfirmed_ratio
    """
...
def auto_assign_cohorts(
    ...
    cohort_profiles: Optional[List[Dict[str, Any]]] = None,  # [{name, max_vms}] Γאפ ramp mode
) -> Dict[str, Any]:
    """
    Propose a tenant Γזע cohort assignment.
    ...
    pilot_bulk     : top-N easiest Γזע "≡ƒº¬ Pilot" cohort; rest Γזע "≡ƒתא Main" cohort
    ...
    max_gb     = guardrails.get("max_disk_tb_per_cohort", 9999) * 1024  # convert TB Γזע GB
    ...
    if cohort_profiles:
        ...
        # ---- pilot_bulk in ramp mode: first pilot_size easiest Γזע slot 1, rest fill slots 2..N ----
            ...
            for t in rest:
                for _ in range(max(num_cohorts - 1, 1)):
                    ...
                    if cur_wave < num_cohorts - 1:
                        ...
                        warnings.append(
                            f"Wave {cur_wave} {reason} hit Γאפ overflow tenant '{t['tenant_name']}' placed in next wave"
                        )
    ...
    if strategy == "balanced_load":
        ...
        for t in sorted_by_gb:
            ...
            if not candidates:
                ...
                warnings.append(f"Guardrail exceeded for tenant '{t['tenant_name']}' Γאפ placed in closest cohort")
    ...
    for t in sorted_easy:
        ...
        for _ in range(num_cohorts):
            ...
            warnings.append(f"Cohort {current_slot} guardrail hit Γאפ overflow tenant '{t['tenant_name']}' placed in next cohort")
...
def build_wave_plan(
    ...
    by_risk      : Green VMs Γזע Wave 1, Yellow Γזע Wave 2, Red Γזע Wave 3.
    ...
    # --- Dependency graph: vm_id Γזע set of vm_ids it depends on ------------
    ...
    if strategy == "pilot_first":
        # Sort by risk asc, then disk asc Γזע take first N as pilot
        ...
        for i, tname in enumerate(ordered_tenants, 1):
            ...
            for part_n, chunk in enumerate(t_chunks, 1):
                ...
                waves_spec.append({
                    "name": f"{wave_name_prefix} {len(waves_spec) + 1} Γאפ {tname}{suffix}",
                    "wave_type": "regular",
                    "vm_ids": [v["id"] for v in chunk if v.get("id")],
                })

    elif strategy == "by_risk":
        # Green Γזע Wave 1, Yellow Γזע Wave 2, Red Γזע Wave 3+
        ...
        for band, label in [("GREEN", "≡ƒƒó Low Risk"), ("YELLOW", "≡ƒƒí Medium Risk"), ("RED", "≡ƒפ┤ High Risk")]:
            ...
            for part_n, chunk in enumerate(chunks, 1):
                ...
                waves_spec.append({
                    "name": f"{wave_name_prefix} {len(waves_spec) + 1} Γאפ {label}{suffix}",
                    "wave_type": "regular",
                    "vm_ids": [v["id"] for v in chunk if v.get("id")],
                })
    ...
    # -----------------------------------------------------------------------
    #  Dependency check Γאפ flag cross-wave dependency violations
    # -----------------------------------------------------------------------
        ...
        for dep_id in deps:
            ...
            if dep_wave is not None and dep_wave >= my_wave:
                warnings.append(
                    ...
                    f"which is in Wave {dep_wave + 1} Γאפ dependency may not be satisfied."
                )
...
# ---------------------------------------------------------------------------
# Phase 5.0 Γאפ Tech Fix Time Scoring
# ---------------------------------------------------------------------------
...
def compute_vm_fix_time(vm: dict, settings: Optional[dict] = None) -> dict:
    ...
    Returns a dict with:
      fix_minutes_raw   Γאפ full score assuming this VM needs a fix
      fix_minutes       Γאפ adjusted by OS-family fix rate (expected value)
      cutover_minutes   Γאפ downtime for final sync + first boot
      downtime_minutes  Γאפ cutover + fix_minutes
      factors           Γאפ list of {name, minutes, reason} contributing items
      is_override       Γאפ True if tech_fix_minutes_override was used
    """
    ...
    if os_family == "windows":
        ...
        factors.append({"name": "Windows OS", "minutes": m,
                         "reason": "NIC adapter rename (vmxnet3Γזעvirtio), disk letter reassignment"})
        total += m
...
def detect_auto_dependencies(
    ...
    1. **RDM disks** Γאפ Any VM with rdm_disk_count > 0 likely shares a LUN with
       other VMs in the same tenant.  We create mutual dependencies between all
       such RDM-using VMs within a tenant (confidence 0.95).

    2. **Shared datastore heuristic** Γאפ VMs that share a datastore whose name
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        ...
        # Group by tenant_id Γאפ all RDM VMs within a tenant are mutual dependents
        from itertools import groupby as _groupby
...
# ====================================================================
# PHASE T.C Γאפ MAINTENANCE WINDOW HELPERS
# ====================================================================
...
def next_window_start(windows: List[dict], after_date) -> Optional[dict]:
    ...
    for day_offset in range(14):
        ...
        dow = candidate.weekday()  # 0=Mon Γאª 6=Sun
        ...
        for w in windows:
            ...
            try:
                ...
                # parse start/end Γאפ may be time objects already (psycopg2) or strings
