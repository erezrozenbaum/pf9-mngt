...
Write-Host "  Password: REDACTED_PASSWORD (for all users)" -ForegroundColor White
...
Write-Host ""
Write-Host "All users have password: REDACTED_PASSWORD" -ForegroundColor Yellow
...
foreach ($user in $users) {
    $loginTest = curl -s -X POST -H "Content-Type: application/json" -d "{`"username`":`"$user`",`"password`":`"REDACTED_PASSWORD`"}" http://localhost:8000/auth/login 2>$null
