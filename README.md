...
[![Version](https://img.shields.io/badge/version-1.94.10-blue.svg)](CHANGELOG.md)
...
### 🏢 Tenant Self-Service Portal *(v1.84.0+, latest v1.94.10)*
...
### Critical tenant permissions bugfix — v1.94.10

**[v1.94.10](CHANGELOG.md)** — Critical bugfix resolving tenant portal permissions error when accessing snapshot data for chargeback calculations. Fixed "permission denied for table metering_snapshots" by using tenant-accessible snapshots table while maintaining comprehensive cost calculations and respecting role-based access controls.

### Critical tenant chargeback bugfix — v1.94.9

**[v1.94.9](CHANGELOG.md)** — Critical bugfix resolving 500 internal server errors in tenant portal chargeback functionality. Fixed database query error related to metering_snapshots table column structure and restored proper snapshot cost calculations with project-level aggregation.

### Comprehensive tenant chargeback enhancement — v1.94.8

**[v1.94.8](CHANGELOG.md)** — Major enhancement bringing tenant UI chargeback calculations to feature parity with admin UI. Now includes comprehensive cost calculations for all resource types (compute, storage, snapshots, networks) with detailed per-VM cost breakdown and category-based cost summaries. Resolves pricing discrepancies between admin and tenant views.

### Enhanced chargeback system and metering fixes — v1.94.5

**[v1.94.5](CHANGELOG.md)** — Complete chargeback overhaul with multi-resource support: (1) **Critical chargeback bug fix** — resolved zero-cost calculations by fixing metering worker to capture VM flavor data when monitoring service returns incomplete information. (2) **Comprehensive cost calculations** — support for all resource types (VMs, Storage, Network, Snapshots) with period-based analysis (7d, 30d, 90d, custom ranges). (3) **Multi-currency support** — enhanced API with ILS pricing configuration and proper currency alignment. (4) **Database fallback logic** — smart metering worker automatically uses database queries when monitoring service lacks flavor data.

### Dark mode fixes and chargeback enhancements — v1.94.3

**[v1.94.3](CHANGELOG.md)** — Comprehensive fixes and enhancements: (1) **Dark mode dependency graphs** — improved edge visibility in Kubernetes dependency graphs using lighter CSS values for better contrast. (2) **Enhanced chargeback system** — new per-VM details endpoint with cost attribution, currency selection support, and expanded VM state collection including stopped/suspended VMs. (3) **Tenant portal UI modernization** — enhanced dashboard and overview screens with improved loading states and theme-aware styling. (4) **CI package synchronization** — resolved npm ci failures by regenerating package-lock.json with missing dependencies.

### Enhanced theme system and UI polish — v1.94.2

**[v1.94.2](CHANGELOG.md)** — UI consistency and theme improvements: (1) **Docs layout optimization** — removed unnecessary right details panel from documentation page for full-width content display. (2) **Dark mode dependency graphs** — fixed edge visibility issues using theme-aware CSS variables instead of hard-coded colors. (3) **TypeScript build fixes** — resolved import syntax issues in tenant-ui for React 19.2+ compatibility. (4) **Theme toggle component** — added modern theme switching UI with accessibility features. (5) **Unified theme system** — shared CSS custom properties between admin and tenant portals for consistent styling.
...
**Project Status**: Production Ready | **Version**: 1.94.10 | **Last Updated**: May 2026
