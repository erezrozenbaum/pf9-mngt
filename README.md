...
**[v1.83.31](CHANGELOG.md)** — Security: dedicated `integration_key` Fernet secret for integration credentials (4.4); performance: 15 per-request `_ensure_tables()` calls removed from `vm_provisioning_routes` (4.3); reliability: Redis-backed atomic `PerformanceMetrics` with in-memory fallback (4.2).
...
**Project Status**: Production Ready | **Version**: 1.83.27 | **Last Updated**: April 2026
