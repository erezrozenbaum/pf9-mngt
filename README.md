...
- **v2.12.5** — Bug fix: NetworkPolicy egress for port 22 added to `pf9-api` policy (gated on `api.nodeLogSource=ssh`) — SSH connections from the API pod to KVM nodes were silently dropped by the default-deny egress rule. (May 2026)
- **v2.12.4** — SSH-based Node Logs: `_fetch_via_ssh()` SSHs to each KVM node as `cloud-kvm` and tails `/var/log/pf9/` log files using `sudo tail`. `NODE_LOG_SOURCE=ssh` env var selects this path; credentials from `pf9-ssh-credentials` K8s secret. `_fetch_via_resmgr()` rewritten to return rich diagnostic pseudo-log-lines from the resmgr host object as a fallback. (May 2026)
- **v2.12.3** — Bug fix: Node Logs `HTTP Error 404` when fetching logs — `_fetch_via_resmgr()` was passing the PostgreSQL integer `id` (e.g. `6`) to resmgr instead of the PF9 host UUID stored in `raw_json->'service'->>'host'` (e.g. `6751ba6a-295b-43be-847a-25c3cd149880`). `_get_nodes()` now returns `resmgr_id`; log fetching uses it. All four KVM nodes now return logs. (May 2026)
